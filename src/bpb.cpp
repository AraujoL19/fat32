...
{
    img.seekg(0);
    img.read(reinterpret_cast<char*>(&bpb), sizeof(BPB));
    return img.gcount() == sizeof(BPB);
}

void printBPB(const BPB &bpb)
{
    std::cout << "Jump Instruction: " << std::hex << std::setw(2) << std::setfill('0')
              << static_cast<int>(bpb.BS_jmpBoot[0]) << " "
              << static_cast<int>(bpb.BS_jmpBoot[1]) << " "
              << static_cast<int>(bpb.BS_jmpBoot[2]) << std::dec << std::endl;

    std::cout << "OEM Name: " << std::string(bpb.BS_OEMName, 8) << std::endl;
    ...
    std::cout << "Media Type: 0x" << std::hex << static_cast<int>(bpb.BS_Media) << std::dec << std::endl;
